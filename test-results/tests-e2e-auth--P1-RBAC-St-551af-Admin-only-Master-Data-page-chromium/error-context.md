...
- Name: tests/e2e/auth.spec.ts >> @P1 @RBAC Story-1.2: Route Guard Enforcement >> [P1] should redirect Host role user away from Admin-only Master Data page
- Location: tests/e2e/auth.spec.ts:117:7

# Error details

```
Error: [B1 Blocker] Cannot get test token for role "Host". Ensure the backend exposes /api/auth/test-token in NODE_ENV=test.
```
